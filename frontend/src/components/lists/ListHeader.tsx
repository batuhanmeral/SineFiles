import { useState } from 'react';
import type { ListDetailResponse } from '@/api/lists.api';

interface Props {
  list: ListDetailResponse;
  onToggleLike: () => void;
  isLiking: boolean;
  canShare: boolean;
}

export function ListHeader({
  list,
  onToggleLike,
  isLiking,
  canShare,
}: Props) {
  const [showShareMenu, setShowShareMenu] = useState(false);

  const shareUrl = `${window.location.origin}/lists/${list.id}`;

  const copyToClipboard = async () => {
    try {
      await navigator.clipboard.writeText(shareUrl);
      // Başarılı mesajını göster (toast vs)
      alert('Bağlantı kopyalandı!');
    } catch (err) {
      console.error('Kopyalama başarısız:', err);
    }
  };

  return (
    <div className="border-b border-white/10 bg-surface-raised">
      <div className="mx-auto max-w-4xl px-4 py-8">
        {/* Başlık ve Tür */}
        <div className="mb-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h1 className="text-3xl font-bold text-ink">{list.title}</h1>
              <p className="mt-2 text-sm text-ink-muted">
                {list.type === 'WATCHED' && '✅ İzlenenler'}
                {list.type === 'WATCHLIST' && '📋 İzlenecekler'}
                {list.type === 'FAVORITES' && '❤️ Favoriler'}
                {list.type === 'CUSTOM' && '📌 Özel Liste'}
                {' • '}
                {list.visibility === 'PUBLIC' ? '🌐 Herkese Açık' : '🔒 Özel'}
                {' • '}
                {list.items.length} içerik
              </p>
            </div>

            {/* Aksiyon Butonları */}
            <div className="flex flex-wrap gap-3">
              {/* Beğeni Butonu */}
              <button
                onClick={onToggleLike}
                disabled={isLiking}
                className="flex items-center gap-2 rounded-lg bg-accent/10 px-4 py-2 text-sm font-medium text-accent transition-all hover:bg-accent/20 disabled:opacity-50"
              >
                <span className={list.likedByMe ? '❤️' : '🤍'} />
                <span>{list.likeCount}</span>
              </button>

              {/* Paylaş Butonu */}
              {canShare && (
                <div className="relative">
                  <button
                    onClick={() => setShowShareMenu(!showShareMenu)}
                    className="rounded-lg bg-white/10 px-4 py-2 text-sm font-medium text-ink transition-all hover:bg-white/20"
                  >
                    📤 Paylaş
                  </button>

                  {/* Paylaş Menüsü */}
                  {showShareMenu && (
                    <div className="absolute right-0 top-full mt-2 rounded-lg border border-white/10 bg-surface-raised p-3 shadow-lg">
                      <p className="mb-2 text-xs font-semibold text-ink-muted">
                        Bu listeyi paylaş
                      </p>

                      {/* URL Kopyala */}
                      <div className="mb-2 flex items-center gap-2">
                        <input
                          type="text"
                          readOnly
                          value={shareUrl}
                          className="flex-1 rounded border border-white/10 bg-surface px-2 py-1 text-xs text-ink"
                        />
                        <button
                          onClick={copyToClipboard}
                          className="rounded bg-accent/20 px-2 py-1 text-xs text-accent hover:bg-accent/30"
                        >
                          Kopyala
                        </button>
                      </div>

                      {/* Sosyal Paylaş (isteğe bağlı) */}
                      <div className="flex gap-2 border-t border-white/10 pt-2">
                        <a
                          href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(`${list.title} listemi göz atın!`)}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex-1 rounded bg-white/5 px-2 py-1 text-center text-xs text-ink-muted hover:bg-white/10"
                        >
                          Twitter
                        </a>
                      </div>
                    </div>
                  )}
                </div>
              )}
            </div>
          </div>
        </div>

        {/* Kapak Resmi */}
        {list.coverImage && (
          <div className="mt-6 h-40 overflow-hidden rounded-lg">
            <img
              src={list.coverImage}
              alt={list.title}
              className="h-full w-full object-cover"
            />
          </div>
        )}
      </div>
    </div>
  );
}
